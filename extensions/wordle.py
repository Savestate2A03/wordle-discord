...
class Wordle(commands.Cog):
	...
	async def giveup(self, ctx, *args):
		...
		player  = user.id
		if not game['active']:
			await ctx.send(f'{user.mention} Game hasn\'t started! You can leave with `!leave`.')
			return
		if player in game['players']:
			# all info for a player is contained in
			# this context so it's fine to delete one mid-game.
	...
	async def start(self, ctx, *args):
		# mark the game context as started.
		channel = ctx.channel.id
		user    = ctx.author
		if channel not in self.games:
		...
		game['active'] = True
		await ctx.send(f'Game started with {len(game["players"])} players! Type `!guess` with your answer spoilered. Alternatively, type in `!giveup` to forfeit.')
	...
	async def guess(self, ctx, *args):
		...
		if players[player]['finished']:
			attached_message += f' Sorry! You\'ve run out of guesses.'
